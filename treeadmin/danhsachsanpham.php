...
<?php
  if(isset($_GET["vector"]))
  {
	...
	{
	  if($id_parent!=0) $sql="select id_sp,capnhat from sanpham where id_parent=$id_parent order by capnhat desc";
	  else $sql="select id_sp,capnhat from sanpham order by capnhat desc";
	  ...
	  if(mysql_num_rows($resultvt)!=0)
	  {
		...
		$sql="update sanpham set capnhat='$capnhatdau' where id_sp=$id_cuoi";
		mysql_query($sql);
		$sql="update sanpham set capnhat='$capnhatcuoi' where id_sp=$id_dau";
		mysql_query($sql);
	  }
	}
  }
...
<br>
<center>
<a href="formnhaplieu.php?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>">Th&#234;m s&#7843;n ph&#7849;m m&#7899;i</a>
</center>
<br>
<iframe name="ifKiemduyet" height="0" width="0"></iframe>
<div class="tintop_title">&nbsp;&nbsp;&nbsp;<img src="icon/dstin.gif" width="32" height="32" align="absbottom">&nbsp;Danh S&#225;ch S&#7843;n Ph&#7849;m Trong M&#7909;c: <?php echo $chuoihienthi;?></div>
<p>
        ...
        <tr>
          <td>
		    <table width="100%"  border="0" cellspacing="1" cellpadding="0">
		 	  <?php
				$sql="select * from sanpham where id_parent=$id_parent";
			  ...
			  <tr bgcolor="<?php if(isset($_GET["vector"])){ if($vector=="xuong"&&intval($id_dau)==intval($row["id_sp"])) echo "#FFCCFF";elseif($vector=="len"&&intval($id_cuoi)==intval($row["id_sp"])) echo "#FFCCFF";else echo "#FFFFFF";}else echo "#FFFFFF";?>" onmouseover="this.bgColor='#D7E3FF'" onmouseout="this.bgColor='<?php if(isset($_GET["vector"])){ if($vector=="xuong"&&intval($id_dau)==intval($row["id_sp"])) echo "#FFCCFF";elseif($vector=="len"&&intval($id_cuoi)==intval($row["id_sp"])) echo "#FFCCFF";else echo "#FFFFFF";}else echo "#FFFFFF";?>'">
				<td width="5%" align="center"><?php echo $i;?></td>
				<td id="<?php echo lay_duongdan($row["id_parent"])."&id=".$row["id_sp"];?>" style="cursor:pointer;" onClick="Preview(this);">&nbsp;&nbsp;
				<?php 
				  if(isset($_GET["vector"])){ if($vector=="xuong"&&intval($id_dau)==intval($row["id_sp"])) echo "<span class='doiVitri'>".$row["tensp"]."</span>";
				  elseif($vector=="len"&&intval($id_cuoi)==intval($row["id_sp"])) echo "<span class='doiVitri'>".$row["tensp"]."</span>";
				  else echo $row["tensp"];}else echo $row["tensp"];?>    
				</td>
				<td width="8%" align="center" style="display:none">
				<?php if($quantri<=1){?>
				<input type="checkbox" name="<?php echo $row["id_sp"];?>" <?php if($row["kiemduyet"]==1) echo "checked";?> onClick="doKiemduyet(this)">
				<?php }?>
				</td>
				<td width="10%" align="center" style="display:none">
				<?php if($quantri<=1){?>
				<input type="checkbox" name="<?php echo $row["id_sp"];?>" <?php if($row["tieudiem"]==1) echo "checked";?> onClick="doTieudiem(this)">
				<?php }?>
				</td>
				<td width="10%" align="center">
				<?php if($quantri<=1){?>
				<?php if($i>$vt){?><a href="javascript:window.location='?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>&vt=<?php $tg=$_GET["vt"];echo $tg;?>&vector=len&id=<?php echo $idluu;?>';">&Lambda;</a><?php }else echo "&nbsp;";?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:window.location='?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>&vt=<?php $tg=$_GET["vt"];echo $tg;?>&vector=xuong&id=<?php echo $row["id_sp"];?>';">V</a>
				<?php }?>
				</td>
				<td width="5%" align="center">
				<?php if($quantri<=1){?>
				<a href="javascript:window.location='formsua.php?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>&id_sp=<?php echo $row["id_sp"];?>';">S&#7917;a</a>
				<a href="javascript:if(confirm('B&#7841;n th&#7921;c s&#7921; mu&#7889;n x&#243;a s&#7843;n ph&#7849;m [<?php echo $row["tensp"];?>] ch&#7913;?')) window.location='?id_parent=<?php echo $id_parent;?>&kieunhap=<?php echo $kieunhap;?>&id_sp=<?php echo $row["id_sp"];?>';">X&#243;a</a>
				<?php }?>
				</td>
			  </tr>
			  <?php $i++;$idluu=$row["id_sp"];}else{?>		  
			  <tr>
