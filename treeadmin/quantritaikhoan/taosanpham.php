...
<?php
  ...
  {	
	...
	$muc=$_POST["dsmuc"];
	$id=getid($muc,$link,"id");
	$sql="insert into danhmuc(id_parent,ten,tukhoa,kieunhap) values('$id','$ten','$tukhoa','4')";	mysql_query($sql);
  }
  $sql="select * from danhmuc where id_parent='83'";
  $result=mysql_query($sql);
?>
<br>
<div class="tintop_title">&nbsp;&nbsp;&nbsp;<img src="../icon/dstin.gif" width="32" height="32" align="absbottom">Tạo nhóm sản phẩm</div>
<p>
  ...
  <tr>
    <td>
      <table width="100%" border="0" cellpadding="0" cellspacing="1" class="inTable">
		  ...
		  <form action="../quantritaikhoan/taosanpham.php?status=submit" method="post" name="frmAddnew">
		  	  <tr>
			<td height="40" width="35%">&nbsp;<span class="articleHead">danh sách nhóm sản phẩm</span></td>
			<td height="40">&nbsp;<select name="dsmuc" class="TextBox">
			<?php if(mysql_num_rows($result)!=0) while($row=mysql_fetch_array($result)){?>
			<option><?php echo $row["tukhoa"];?></option>
			<?php }?>
			</select>
			</td>
		  </tr>
		  <tr>
			<td height="40" width="30%">&nbsp;<span class="articleHead">tên của nhóm sản phẩm thêm</span></td>
